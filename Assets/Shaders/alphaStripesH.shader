...
Shader "Custom/alphaStripesH"
{
	Properties
	{
		...
		_StripesCutOff("Stripes CutOff", Range(0,10)) = 0
	}
	...
	Subshader
	{
		...
		Pass
		{
			...
			{
				vertexOutput o;
				o.posObj = float4(v.vertex.xy, v.vertex.z + 3, v.vertex.w);
			...
			fixed4 frag(vertexOutput i): SV_Target
			{
				...
				float3 normalWorldAtPixel;
					...
					//Striping Albedo
					float z = (i.posObj.z/_Stripes*10);
					float zmod = fmod(z,10) >= _StripesCutOff ? 1 : 0;
					if(zmod == 0)
						albedoColor = half4(tex2D(_MainTex, i.texcoord).xyz,1);
					else
						albedoColor = half4(tex2D(_MainTex, i.texcoord).xyz,0);
