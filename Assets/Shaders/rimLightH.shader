...
Shader "Custom/rimLightH"
{
	Properties
	{
		...
		_RimCutOff ("Rim CutOff", Range(0,1)) = 1
	}
	...
	Subshader
	{
		...
		Pass
		{
			...
			float3 RimLight(float3 N, float3 L, float3 V, float3 specularColor, float specularFactor, float attenuation, float rimBorderFactor, float cutoff)
			{
				float dotModified = abs(dot(N,V));
				dotModified = cutoff < dotModified ? dotModified : 0;
				return specularColor * specularFactor * pow(1-dotModified, rimBorderFactor * attenuation);
			}
			...
			fixed4 frag(vertexOutput i): SV_Target
			{
				...
				float3 normalWorldAtPixel;
					...
					// 											attenuation,
					// 											_SpecularPower);
					float3 rimLightVal = RimLight(normalWorldAtPixel, lightDir, V, specularMapColor.xyz, _SpecularFactor, attenuation, _RimBorder, _RimCutOff);
					float rimLightAlpha = (rimLightVal.x + rimLightVal.y + rimLightVal.z)/3;
					#if _AMBIENTMODE_ON
						ambientColor = i.ambientColor;
					#endif

					finalColor = float4(rimLightVal * _RimColor,
										albedoColor.a * _Color.a + rimLightAlpha);
				#elif _LIGHTING_VERT
