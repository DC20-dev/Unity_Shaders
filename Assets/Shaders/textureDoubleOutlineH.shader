...
Shader "Custom/textureDoubleOutlineH"
{
	Properties{
		_Color("First Color", Color) = (1,1,1,1)
		_SecondColor("SecondColor", Color) = (1,0,0,1)
		_MainTex("MainTexture", 2D) = "white" {}
		_FirstBorder("First Border", Range(0,1)) = 0
		_SecondBorder("SecondBorder", Range(0,1)) = 0
	}
	...
	Subshader
	{
		...
		Pass
		{
			...
			vertexOutput vert(vertexInput v)
			{
				...
				float4x4 scaleM = float4x4(
					1+_FirstBorder, 0, 0, 0,
					0, 1+_FirstBorder, 0, 0,
					0, 0, 1+_FirstBorder, 0,
					0, 0, 0, 1
				);
		...
		Pass
		{
			...
			vertexOutput vert(vertexInput v)
			{
                vertexOutput o;
				float4x4 scaleM = float4x4(
					1+_SecondBorder, 0, 0, 0,
					0, 1+_SecondBorder, 0, 0,
					0, 0, 1+_SecondBorder, 0,
					0, 0, 0, 1
				);
				float4 scaledObjPos = mul( scaleM ,v.vertex);
                o.pos = mul(UNITY_MATRIX_MVP, scaledObjPos);  //projection space
